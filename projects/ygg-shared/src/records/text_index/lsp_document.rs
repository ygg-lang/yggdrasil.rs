...
impl<T: Borrow<str>> IndexedText<T> {
    pub fn new(text: T) -> Self {
        ...
        while let Some((pos, c)) = char_iter.next() {
            ...
            if is_newline {
                ...
                debug_assert!(text.borrow().is_char_boundary(start), "Start is not at char boundary");
                let end = pos + c.len_utf8();
                debug_assert!(text.borrow().is_char_boundary(end), "End is not at char boundary");
    ...
    fn pos_to_offset(&self, pos: &LineColumn) -> Option<usize> {
