    ...
    Inputs: Iterator<Item = Self::Input>,
{
    ...
    fn parse_input(&mut self, tail: &mut Peekable<&mut Inputs>, rbp: Precedence) -> Result<Self::Output> {
        if let Some(head) = tail.next() {
            ...
            Err(Error::unexpected_token("EmptyInput", None, None))
        }
    }
    ...
    fn nud(&mut self, head: Self::Input, tail: &mut Peekable<&mut Inputs>, info: Affix) -> Result<Self::Output> {
        match info {
            ...
            Affix::Suffix(_) => Err(Error::unexpected_token("Unexpected Postfix", None, None)),
            Affix::Infix(_, _) => Err(Error::unexpected_token("Unexpected Infix", None, None)),
        }
    }
    ...
    fn led(
        ...
        match info {
            ...
            Affix::None => Err(Error::unexpected_token("Unexpected NilFix", None, None)),
            Affix::Prefix(_) => Err(Error::unexpected_token("Unexpected Prefix", None, None)),
        }
    }
