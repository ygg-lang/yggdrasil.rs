...
mod tests {
    ...
    fn get() {
        let input = "abc123abc";
        let span = TextSpan::new(input, 3, input.len()).unwrap();
    ...
    fn get_fails() {
        let input = "abc";
        let span = TextSpan::new(input, 0, input.len()).unwrap();
    ...
    fn span_comp() {
        let input = "abc\ndef\nghi";
        let span = TextSpan::new(input, 1, 7).unwrap();
        let span2 = TextSpan::new(input, 50, 51);
        assert!(span2.is_none());
        let span3 = TextSpan::new(input, 0, 8).unwrap();
        assert!(span != span3);
    }
    ...
    fn lines_eof() {
        let input = "abc\ndef\nghi";
        let span = TextSpan::new(input, 5, 11).unwrap();
    ...
    fn lines_span() {
        ...
        assert_eq!(lines_span[0], TextSpan::new(input, 0, 4).unwrap());
        assert_eq!(lines_span[1], TextSpan::new(input, 4, 8).unwrap());
        assert_eq!(lines_span.iter().map(|span| span.as_str()).collect::<Vec<_>>(), lines);
    }
