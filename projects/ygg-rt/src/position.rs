...
impl<'i> Position<'i> {
    ...
    pub(crate) fn skip_until(&mut self, strings: &[&str]) -> bool {
        ...
        #[cfg(feature = "memchr")]
        {
            match strings {
                [] => (),
                [s1] => {
                    if let Some(from) = memchr::memmem::find(&self.input.as_bytes()[self.position..], s1.as_bytes()) {
                        self.position += from;
                        return true;
                    }
                }
                [s1, s2] if !s1.is_empty() && !s2.is_empty() => {
                    ...
                    let miter = memchr::memchr2_iter(b1, b2, &self.input.as_bytes()[self.position..]);
                    for from in miter {
                        let start = &self.input[self.position + from..];
                ...
                [s1, s2, s3] if !s1.is_empty() && !s2.is_empty() && s3.is_empty() => {
                    ...
                    let miter = memchr::memchr3_iter(b1, b2, b3, &self.input.as_bytes()[self.position..]);
                    for from in miter {
                        let start = &self.input[self.position + from..];
    ...
    fn skip_until_basic(&mut self, strings: &[&str]) -> bool {
        ...
        for from in self.position..self.input.len() {
...
mod tests {
    ...
    fn line_col() {
        ...
        assert_eq!(Position::new(input, 0).unwrap().line_column(), (1, 1));
        assert_eq!(Position::new(input, 1).unwrap().line_column(), (1, 2));
        assert_eq!(Position::new(input, 2).unwrap().line_column(), (1, 3));
        assert_eq!(Position::new(input, 3).unwrap().line_column(), (1, 4));
        assert_eq!(Position::new(input, 4).unwrap().line_column(), (2, 1));
        assert_eq!(Position::new(input, 5).unwrap().line_column(), (2, 2));
        assert_eq!(Position::new(input, 6).unwrap().line_column(), (2, 3));
        assert_eq!(Position::new(input, 7).unwrap().line_column(), (3, 1));
        assert_eq!(Position::new(input, 8).unwrap().line_column(), (3, 2));
        assert_eq!(Position::new(input, 11).unwrap().line_column(), (3, 3));
        let input = "abcd嗨";
        assert_eq!(Position::new(input, 7).unwrap().line_column(), (1, 6));
    }
    ...
    fn measure_skip(input: &str, pos: usize, n: usize) -> Option<usize> {
        ...
        if p.skip(n) { Some(p.position - pos) } else { None }
    }
    ...
    fn skip_until() {
        ...
        assert_eq!(test_pos.offset(), 0);

        test_pos = pos;
        test_pos.skip_until(&["b"]);
        assert_eq!(test_pos.offset(), 1);
        ...
        assert_eq!(test_pos.offset(), 3);

        test_pos = pos;
        assert!(!test_pos.skip_until(&["z"]));
        assert_eq!(test_pos.offset(), 5);
    }
