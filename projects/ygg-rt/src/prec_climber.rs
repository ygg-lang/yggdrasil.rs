...
#[derive(Debug)]
pub struct Operator<R: YggdrasilRule> {
    rule: R,
    assoc: Assoc,
    next: Option<Box<Operator<R>>>,
}

impl<R: YggdrasilRule> Operator<R> {
...
impl<R: YggdrasilRule> BitOr for Operator<R> {
    type Output = Self;

    fn bitor(mut self, rhs: Self) -> Self {
        fn assign_next<R: YggdrasilRule>(op: &mut Operator<R>, next: Operator<R>) {
...
impl<R: YggdrasilRule> PrecClimber<R> {
    ...
    pub fn new(ops: Vec<Operator<R>>) -> PrecClimber<R> {
        let ops = ops.into_iter().zip(1..).fold(Vec::new(), |mut vec, (op, prec)| {
        ...
        G: FnMut(T, TokenPair<'i, R>, T) -> T,
    {
        while pairs.peek().is_some() {
            let rule = pairs.peek().unwrap().get_rule();
