...
fn benchmark(b: &mut Criterion) {
    ...
    macro_rules! test_series {
        ($kind:ident) => {
            b.bench_function(stringify!(push - restore - $kind), |b| b.iter(|| snapshot_push_restore($kind.clone())))
                .bench_function(stringify!(push - clear - $kind), |b| b.iter(|| snapshot_push_clear_snapshot($kind.clone())))
                .bench_function(stringify!(pop - restore - $kind), |b| b.iter(|| snapshot_pop_restore($kind.clone())))
                .bench_function(stringify!(pop - clear - $kind), |b| b.iter(|| snapshot_pop_clear($kind.clone())))
        };
    }
    test_series!(small);
    test_series!(medium);
