...
// Hash of the grammar file: 2DAA76E62ADB9B034D98BC4E70CBC6FAB0B4D965D94D5513BAFB3C72A35D0262
...
mod peginator_generated {
    ...
    mod DefineStatement_impl {
        ...
        pub fn parse<'a>(
            ...
            let ParseOk { state, .. } = parse_Whitespace(state, tracer, cache)
                ...
                .discard_result()?;
            let ParseOk { result: define, state } = parse_Whitespace(state, tracer, cache)
                .and_then(|ParseOk { state, .. }| parse_DefineNode(state, tracer, cache))?;
    ...
    #[inline]
    pub(super) fn parse_DefineNode<'a>(
        state: ParseState<'a>,
        tracer: impl ParseTracer,
        cache: &mut ParseCache<'a>,
    ) -> ParseResult<'a, DefineNode> {
        tracer.run_traced("DefineNode", state, |state, tracer| DefineNode_impl::rule_parser(state, tracer, cache))
    }
    mod MacroCalls_impl {
        use super::*;
        mod closure {
    ...
    mod ChoiceNode_impl {
        ...
        mod part_1 {
            ...
            pub fn parse<'a>(
                ...
                loop {
                    match parse_Whitespace(state.clone(), tracer, cache)
                        .and_then(|ParseOk { state, .. }| parse_ExprStream(state, tracer, cache))
                        .map_inner(|result| vec![result])
                    {
                        Ok(ParseOk { result: __result, state: new_state, .. }) => {
