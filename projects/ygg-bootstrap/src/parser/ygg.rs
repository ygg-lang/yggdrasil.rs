...
// Hash of the grammar file: D6B9D565AA35AD7678EDEF1C186CA0344DC6B9E036F9C4592B41C370979CE09F
...
mod peginator_generated {
    ...
    mod DefineStatement_impl {
        ...
        pub fn parse<'a>(state: ParseState<'a>, tracer: impl ParseTracer, cache: &mut ParseCache<'a>) -> ParseResult<'a, Parsed> {
            ...
            match part_7::parse(state, tracer, cache) {
                Ok(ParseOk {
                    result: _,
                    state: new_state,
                    farthest_error: new_farthest_error,
    ...
    mod IdentifierNode_impl {
        ...
        mod choice_2 {
            ...
            mod optional {
                ...
                pub fn parse<'a>(state: ParseState<'a>, tracer: impl ParseTracer, cache: &mut ParseCache<'a>) -> ParseResult<'a, Parsed> {
                    let ok_result = parse_XID_START(state, tracer, cache)?;
                    Ok(ok_result.map(|result| Parsed { label: Some(result) }))
                }
                pub struct Parsed {
                    pub label: Option<XID_START>,
                }
            }
            #[inline(always)]
            pub fn parse<'a>(state: ParseState<'a>, tracer: impl ParseTracer, cache: &mut ParseCache<'a>) -> ParseResult<'a, Parsed> {
                match optional::parse(state.clone(), tracer, cache) {
                    Ok(ok_result) => Ok(ok_result.map(|result| Parsed { label: result.label })),
                    Err(err) => Ok(ParseOk {
                        result: Parsed { label: Default::default() },
                        state,
                        farthest_error: Some(err),
                    }),
                }
        ...
        pub fn parse<'a>(state: ParseState<'a>, tracer: impl ParseTracer, cache: &mut ParseCache<'a>) -> ParseResult<'a, Parsed> {
            ...
            let mut farthest_error: Option<ParseError> = None;
            match choice_0::parse(state.clone(), tracer, cache) {
                Ok(ok_result) => return Ok(ok_result.map(|result| Parsed { label: None })),
                Err(err) => farthest_error = combine_errors(farthest_error, Some(err)),
            }
            match choice_1::parse(state.clone(), tracer, cache) {
                Ok(ok_result) => return Ok(ok_result.map(|result| Parsed { label: None })),
            ...
            match choice_2::parse(state.clone(), tracer, cache) {
                Ok(ok_result) => return Ok(ok_result.map(|result| Parsed { label: result.label })),
                Err(err) => farthest_error = combine_errors(farthest_error, Some(err)),
            }
            match choice_3::parse(state.clone(), tracer, cache) {
                Ok(ok_result) => return Ok(ok_result.map(|result| Parsed { label: None })),
        ...
        #[inline(always)]
        pub fn rule_parser<'a>(state: ParseState<'a>, tracer: impl ParseTracer, cache: &mut ParseCache<'a>) -> ParseResult<'a, IdentifierNode> {
            let result = parse(state.clone(), tracer, cache)?.map_with_state(|_, new_state| {
                let string = state.slice_until(new_state).to_string();
    ...
    #[inline]
    pub(super) fn parse_IdentifierNode<'a>(state: ParseState<'a>, tracer: impl ParseTracer, cache: &mut ParseCache<'a>) -> ParseResult<'a, IdentifierNode> {
