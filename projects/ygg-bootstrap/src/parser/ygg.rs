...
// Hash of the grammar file: 0F0088912772BCFD6BA21D390CD41FBB47DDAD302C5B9022CD3658FE2EBF2B14
...
pub type CharItem = Option<CharRange>;
#[derive(Debug, Clone)]
    ...
    mod CharItem_impl {
        ...
        mod choice_0 {
            ...
            pub fn parse<'a>(state: ParseState<'a>, tracer: impl ParseTracer, cache: &mut ParseCache<'a>) -> ParseResult<'a, Parsed> {
                let ParseOk { state, .. } = parse_Whitespace(state, tracer, cache)?;
                let ok_result = parse_CharRange(state, tracer, cache)?;
                Ok(ok_result.map(|result| Parsed { _override: Some(result) }))
            }
            pub struct Parsed {
                pub _override: Option<CharRange>,
            }
        }
        mod choice_1 {
            use super::*;
        ...
        pub fn parse<'a>(state: ParseState<'a>, tracer: impl ParseTracer, cache: &mut ParseCache<'a>) -> ParseResult<'a, Parsed> {
            ...
            match choice_1::parse(state.clone(), tracer, cache) {
                Ok(ok_result) => return Ok(ok_result.map(|result| Parsed { _override: None })),
                Err(err) => farthest_error = combine_errors(farthest_error, Some(err)),
            }
            Err(farthest_error.unwrap_or_else(|| state.report_error(ParseErrorSpecifics::Other)))
        }
